...
public class JsonRpcHandler {
  ...
  public JSONObject process(HttpServletRequest servletRequest, JSONObject request) throws RpcException, JSONException {
    ...
    // Add some viewer data in the form of
    response.put("viewer", getViewerForRequest(servletRequest));
    
    return response;
  }

  /**
   * Return a security token for this gadget, subclass this to parse your cookeies, etc.
   * @param req
   * @return
   */
  ...
  protected JSONObject getViewerForRequest(HttpServletRequest req) {
    try {
      JSONObject name = new JSONObject().put("unstructured", "Fred Flintstone");
      return new JSONObject().put("id", "100").put("name", name);
    } catch (JSONException e) {
      return null;
    }
  }

  protected JSONObject getOwnerForRequest(HttpServletRequest req) {
    return null;
  }
